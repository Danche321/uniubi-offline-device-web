<template>
  <el-form ref="form" :model="form" class="from" :rules="rules">
    <el-form-item :label="$t('main_new_password_password_password1_label')" prop="password1">
      ...
      <p>{{$t('main_new_password_password_password1_tip')}}</p>
    </el-form-item>
    <el-form-item :label="$t('main_new_password_password_password2_label')" prop="password2">
    ...
    <el-button type="primary" @click="nextStep" class="btn-next">{{$t('main_new_password_password_submit_text')}}</el-button>
  </el-form>
</template>
    ...
    nextStep () {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          this.$emit('nextStep', JSON.stringify(params))
        }
      })
    }
  }
